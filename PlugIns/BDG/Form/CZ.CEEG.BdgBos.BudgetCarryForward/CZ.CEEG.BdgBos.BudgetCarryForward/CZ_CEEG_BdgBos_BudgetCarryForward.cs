...
namespace CZ.CEEG.BdgBos.BudgetCarryForward
{
    [Description("月度预算结转")]
    [HotUpdate]
    public class CZ_CEEG_BdgBos_BudgetCarryForward : AbstractBillPlugIn
    {
        #region override
        ...
        private void Act_CarryForward()
        {
            ...
            if (DateTime.Now.Month != 12)
            {
                ...
                if(_FIsTurn == "False")
                {
                    ...
                    string sql = string.Format("exec proc_czly_BugetCarryForward @FIDB='{0}',@FCreatorId='{1}',@FCreateOrgId='{2}'",
                        _FID, _FCreatorId, _FCreateOrgId);
                    CZDB_GetData(sql);
                    this.View.Refresh();
                    this.View.ShowMessage("本月预算结转完成！");
                }
                else
                {
                    this.View.ShowMessage("本月预算已经结转到下月！");
                }
            }
            else
            {
