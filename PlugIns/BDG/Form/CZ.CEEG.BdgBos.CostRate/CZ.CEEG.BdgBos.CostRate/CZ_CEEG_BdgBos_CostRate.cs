...
namespace CZ.CEEG.BdgBos.CostRate
{
    [Description("费用系数")]
    [HotUpdate]
    public class CZ_CEEG_BdgBos_CostRate : AbstractBillPlugIn
    {
        #region override
        ...
        private bool Act_Check()
        {
            ...
            string sql = string.Format("select * from ora_BDG_CostRate where FYear='{0}' and FBraOffice='{1}' and FDocumentStatus in ('B','C')",
                                        _FYear, _FBraOffice);
            var obj = CZDB_GetData(sql);
            if (obj.Count > 0)
            {
                this.View.ShowMessage("此公司当前年度的费用系数表已经存在!");
                return true;
            }
            return false;
        }
