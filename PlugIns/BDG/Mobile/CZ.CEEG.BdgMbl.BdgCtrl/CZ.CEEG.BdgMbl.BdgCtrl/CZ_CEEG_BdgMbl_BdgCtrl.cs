...
namespace CZ.CEEG.BdgMbl.BdgCtrl
{
    [Description("预算占用控制")]
    [HotUpdate]
    public class CZ_CEEG_BdgMbl_BdgCtrl : AbstractMobileBillPlugin
    {
        ...
        public override void DataChanged(DataChangedEventArgs e)
        {
            ...
            if (IsUsingBdgSys())
            {
                ...
                if (e.Field.Key == FieldName["FCostPrj"])
                {
                    ...
                    string FCostPrj = this.View.BillModel.GetValue(FieldName["FCostPrj"], e.Row) == null ? "" : (this.View.BillModel.GetValue(FieldName["FCostPrj"], e.Row) as DynamicObject)["Id"].ToString();
        ...
        private bool Check()
        {
            ...
            if (objs.Count > 0)
            {
                ...
                {
                    //获取单据中总预计金额
                    float FTPreCost = float.Parse(this.View.BillModel.GetValue(fieldname["FTPreCost"]).ToString());
                    //float FTReCost = float.Parse(this.View.BillModel.GetValue(fieldname["FTReCost"]).ToString());
                ...
                else if(GetCtrlStrategy(FBraOffice) == 1) //分控
                {
                    ...
                    {
                        FCostPrj = data[fieldname["FCostPrj"]] == null ? "" : (data[fieldname["FCostPrj"]] as DynamicObject)["Id"].ToString();
        ...
        public DynamicObjectCollection DB_GetFormDatas()
        {
            ...
            if (hasEntry)
            {
                datas = this.View.BillModel.DataObject["FEntity"] as DynamicObjectCollection;
            }
            else
            {
                BusinessInfo info = this.View.BillView.BusinessInfo;
                datas = new DynamicObjectCollection(this.View.BillModel.DataObject.DynamicObjectType);
                datas.Add(this.View.BillModel.DataObject);
            }
            return datas;
        }
        ...
        {
            return this.View.BillModel.DataObject[sign] == null ? "0" : (this.View.BillModel.DataObject[sign] as DynamicObject)[property].ToString();
        }
        /// <summary>
        /// 获取一般字段
        /// </summary>
