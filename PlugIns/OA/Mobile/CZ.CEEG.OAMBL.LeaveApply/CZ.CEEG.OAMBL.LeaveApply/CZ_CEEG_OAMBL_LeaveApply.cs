...
namespace CZ.CEEG.OAMBL.LeaveApply
{
    [Description("请假控制")]
    [HotUpdate]
    public class CZ_CEEG_OAMBL_LeaveApply : AbstractMobileBillPlugin
    {
        #region 新增时设置由表头带出表体的请假人
        /// <summary>
        /// 新增时设置由表头带出表体的请假人（暂定）
        /// </summary>
        private void addEntryLeaver()
        {
            if (this.View.BillModel.GetValue("FDocumentStatus").ToString() == "Z")
            {
                string _FApplyID = this.View.BillModel.DataObject["FApplyID"] == null ? "" : (this.View.BillModel.DataObject["FApplyID"] as DynamicObject)["Id"].ToString();
                string _F_ora_Post = this.View.BillModel.DataObject["F_ora_Post"] == null ? "" : (this.View.BillModel.DataObject["F_ora_Post"] as DynamicObject)["Id"].ToString();
                string _FDeptID = this.View.BillModel.DataObject["FDeptID"] == null ? "" : (this.View.BillModel.DataObject["FDeptID"] as DynamicObject)["Id"].ToString();

                int Row = this.View.BillModel.GetEntryRowCount("FEntity") - 1;
                //this.View.ShowMessage(Row.ToString());
                if (_FApplyID != "" && _F_ora_Post != "" && _FDeptID != "")
                {
                    this.View.BillModel.SetItemValueByID("FName", _FApplyID, Row);
                    this.View.BillModel.SetItemValueByID("FPost", _F_ora_Post, Row);
                    this.View.BillModel.SetItemValueByID("FDept", _FDeptID, Row);
                }
            }
        }
        ...
        {
            string FApplyID = this.View.BillModel.GetValue("FName", Row) == null ? "" : (this.View.BillModel.GetValue("FName", Row) as DynamicObject)["Id"].ToString();
            ...
            this.View.BillModel.SetItemValueByID("FPost", _FPostID, Row);
            this.View.BillModel.SetItemValueByID("FDept", _FDeptID, Row);
        }
        #endregion
        ...
        {
            string leaveType = this.View.BillModel.GetValue("FLeaveType", e.Row).ToString();
            ...
            {
                string orgId = this.View.BillModel.DataObject["FOrgID"] == null ? "" : (this.View.BillModel.DataObject["FOrgID"] as DynamicObject)["Id"].ToString();
                string startDate = this.View.BillModel.GetValue("FStartDate", e.Row).ToString();
                string endDate = this.View.BillModel.GetValue("FEndDate", e.Row).ToString();
                string startTimeFrame = this.View.BillModel.GetValue("FStartTimeFrame", e.Row).ToString();
                string endTimeFrame = this.View.BillModel.GetValue("FEndTimeFrame", e.Row).ToString();
                string sTime = this.View.BillModel.GetValue("FSTime", e.Row).ToString();
                string eTime = this.View.BillModel.GetValue("FETime", e.Row).ToString();
                ...
                if (startDate != "" && startTimeFrame != "" && endDate != "" && endTimeFrame != "" && orgId != "")
                {
                    ...
                    {
                        this.View.BillModel.SetValue("FSTime", AM_S, e.Row);
                    }
                    else if (e.Field.Key == "FStartTimeFrame" && startTimeFrame == "2")
                    {
                        this.View.BillModel.SetValue("FSTime", PM_S, e.Row);
                    }
                    //根据结束时段上下午设置结束时间
                    if (e.Field.Key == "FEndTimeFrame" && endTimeFrame == "1")
                    {
                        this.View.BillModel.SetValue("FETime", AM_E, e.Row);
                    }
                    else if (e.Field.Key == "FEndTimeFrame" && endTimeFrame == "2")
                    {
                        this.View.BillModel.SetValue("FETime", PM_E, e.Row);
                    }
                    //同步时间和时段2 : 如果改动的是时间
                    ...
                    {
                        this.View.BillModel.SetValue("FEndTimeFrame", "1", e.Row);
                        endTimeFrame = "1";
                        this.View.UpdateView("F_ora_MobileProxyEntryEntity");
                    }
                    else if (e.Field.Key == "FETime" && DateTime.Parse(eTime).CompareTo(md) >= 0)
                    {
                        this.View.BillModel.SetValue("FEndTimeFrame", "2", e.Row);
                        endTimeFrame = "2";
                        this.View.UpdateView("F_ora_MobileProxyEntryEntity");
                    }
                    ...
                    {
                        startTimeFrame = this.View.BillModel.GetValue("FStartTimeFrame", e.Row).ToString();
                        endTimeFrame = this.View.BillModel.GetValue("FEndTimeFrame", e.Row).ToString();
                        sTime = this.View.BillModel.GetValue("FSTime", e.Row).ToString();
                        eTime = this.View.BillModel.GetValue("FETime", e.Row).ToString();
        ...
        public override void AfterBindData(EventArgs e)
        {
            ...
            {
                isFirstOpen = false;
                string type = this.View.BillModel.GetValue("FLeaveType", 0).ToString();
                string msg = QueryLeftDays(type, 0, true);
        ...
        public override void DataChanged(DataChangedEventArgs e)
        {
            ...
            if (e.Field.Key == "FLeaveType" || e.Field.Key == "FName")
            {
                ...
                string type = this.View.BillModel.GetValue("FLeaveType", e.Row).ToString();
                string msg = "";
        ...
        public override void BeforeDoOperation(BeforeDoOperationEventArgs e)
        {
            ...
            if(_opKey == "SUBMIT")
            {
                ...
                //设置最大请假天数
                this.View.BillModel.SetValue("FLTDays", GetMaxLeaveDay());
            }
        }
        #endregion

        #region 获取最大请假天数
        private float GetMaxLeaveDay()
        {
            var entity = this.View.BillModel.DataObject["FEntity"] as DynamicObjectCollection;
            float MaxDay = 0;
            float temDay = 0;
        ...
        private bool Check()
        {
            var _FEntity = this.View.BillModel.DataObject["FEntity"] as DynamicObjectCollection;
            string _FLeaveType = "";
            string _FStartDate = "";
            ...
            foreach (string leaver in leavers)
            {
                ...
                for(int i = 0; i < _FEntity.Count; i++)
                {
                    ...
                    if (leaver == _FName && _FLeaveType != "9") //不为调休
                    {
                        ...
                        //text += _FLeaveType + "#" + _FStartDate + "#" + _FStartTimeFrame + "#" + _FEndDate + "#" + _FEndTimeFrame + "#" + _FDayNum + "#,";
                    ...
                    else if (leaver.Equals(_FName) && _FLeaveType == "9")
                    {
                        ...
                        if (float.Parse(_FDayNum) * 8.0 > float.Parse(_FLeftHours))
                        {
                            ...
                            this.View.ShowErrMessage(name + "超出了可调休的时长！");
                            return true;
                        }
                    }
                }
        ...
        private bool CheckByType(DynamicObject obj_i, string _FNAME)
        {
            ...
            switch (type)
            {
                case "1":
                    ...
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的产检假提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "2":
                    ...
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的事假提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    else if (applyDays > 5)
                    {
                ...
                case "3":
                    ...
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的探亲假提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "4":
                    if (applyDays > (_FAllowDays - _FHisDays))
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的病假提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "5":
                    if (applyDays > (_FAllowDays - _FHisDays))
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的护理假提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "6":
                    ...
                    if (applyDays > (_FAllowDays - _FHisDays - tempHisDays))
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的年休假提交失败！\n原因：\n超出了可请假天数！");
                        return true;
                    }
                    break;
                case "7":
                    ...
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的丧假提交失败！\n原因：\n超出了本次可请假天数！");
                        return true;
                    }
                    break;
                case "8":
                    ...
                    if (applyDays > _FAllowDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的婚假提交失败！\n原因：\n超出了本次可请假天数！");
                        return true;
                    }
                    break;
                case "9":
                ...
                case "12":
                    if (applyDays > _FAllowDays - _FHisDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的顺产假提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "13":
                    if (applyDays > _FAllowDays - _FHisDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的剖腹产假提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "14":
                    if (applyDays > _FAllowDays - _FHisDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的流产假(90天内)提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "15":
                    if (applyDays > _FAllowDays - _FHisDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的流产假(210天内)提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "16":
                    if (applyDays > _FAllowDays - _FHisDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的流产假(210天以上)提交失败！\n原因：\n超出了剩余可请假天数！");
                        return true;
                    }
                    break;
                case "17":
                ...
                case "18":
                    if (applyDays > _FAllowDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的拆迁假提交失败！\n原因：\n超出了本次可请假天数！");
                        return true;
                    }
                    break;
                case "19":
                    if (applyDays > _FAllowDays)
                    {
                        this.View.ShowErrMessage(name + "第" + row + "行的献血假提交失败！\n原因：\n超出了本次可请假天数！");
                        return true;
                    }
                    break;
                default:
        ...
        {
            string _FNAME = this.View.BillModel.GetValue("FNAME", Row) == null ? "0" : (this.View.BillModel.GetValue("FNAME", Row) as DynamicObject)["Id"].ToString();
            ...
            * */
            ...
            switch (_FLeaveType)
            {
                ...
                case "3":
                    ...
                    tempHisDays = float.Parse(GetQuery("6", _FNAME, Row)[1]["FHisDays"].ToString());
                    // 判断是否为销售员，销售员不显示请假天数
                ...
                case "6":
                    ...
                    tempHisDays = float.Parse(GetQuery("3", _FNAME, Row)[1]["FHisDays"].ToString());
        ...
        {
            return (this.View.BillModel.DataObject as DynamicObject)["Id"] == null ? "0" : (this.View.Model.DataObject as DynamicObject)["Id"].ToString();
        }
        /// <summary>
        /// 获取基础资料
        /// </summary>
        ...
        {
            return this.View.BillModel.DataObject[sign] == null ? "" : (this.View.Model.DataObject[sign] as DynamicObject)[property].ToString();
        }
        /// <summary>
        /// 获取一般字段
        /// </summary>
        ...
        {
            return this.View.BillModel.DataObject[sign] == null ? "" : this.View.Model.DataObject[sign].ToString();
        }
        #endregion
