...
from(select distinct s.FBillNO,se.FMTLGROUP 
	...
	from(select FID,FBillNo from ora_CRM_SaleOffer where FID=@FCrmSOID)s 
	inner join ora_CRM_SaleOfferEntry se on s.FID=se.FID
...
-- exec proc_cztyCrm_VadOffer @FCrmSOID=110170 
