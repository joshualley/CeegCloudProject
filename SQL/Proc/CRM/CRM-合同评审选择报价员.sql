...
from(select distinct FMtlGroup from ora_CRM_ContractEntry where FID=@FCrmSOID)sb 
...
update ora_CRM_Contract set FChecker1=(select FUserID from #u where FSEQ=1) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=2)
update ora_CRM_Contract set FChecker2=(select FUserID from #u where FSEQ=2) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=3)
update ora_CRM_Contract set FChecker3=(select FUserID from #u where FSEQ=3) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=4)
update ora_CRM_Contract set FChecker4=(select FUserID from #u where FSEQ=4) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=5)
update ora_CRM_Contract set FChecker5=(select FUserID from #u where FSEQ=5) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=6)
update ora_CRM_Contract set FChecker6=(select FUserID from #u where FSEQ=6) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=7)
update ora_CRM_Contract set FChecker7=(select FUserID from #u where FSEQ=7) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=8)
update ora_CRM_Contract set FChecker8=(select FUserID from #u where FSEQ=8) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=9)
update ora_CRM_Contract set FChecker9=(select FUserID from #u where FSEQ=9) where FID=@FCrmSOID
if exists(select * from #u where FSEQ=10)
update ora_CRM_Contract set FChecker10=(select FUserID from #u where FSEQ=10) where FID=@FCrmSOID
...
-------------------
-- exec proc_cztyCrm_RndContract @FCrmSOID=110170 
